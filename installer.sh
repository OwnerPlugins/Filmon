...
echo "Starting Filmon installation..."

if [ ! -d /usr/lib64 ]; then
    PLUGINPATH=/usr/lib/enigma2/python/Plugins/Extensions/Filmon
else
    PLUGINPATH=/usr/lib64/enigma2/python/Plugins/Extensions/Filmon
fi
...
cleanup() {
    ...
    [ -d "/tmp/Filmon-main" ] && rm -rf "/tmp/Filmon-main"
}
...
echo "Downloading Filmon..."
wget --no-check-certificate 'https://github.com/Belfagor2005/Filmon/archive/refs/heads/main.tar.gz' -O "$FILEPATH"
if [ $? -ne 0 ]; then
    echo "Failed to download Filmon package!"
    cleanup
    exit 1
fi
...
if [ $? -ne 0 ]; then
    echo "Failed to extract Filmon package!"
    cleanup
    exit 1
fi
...
if [ -d "$TMPPATH/Filmon-main/usr/lib/enigma2/python/Plugins/Extensions/Filmon" ]; then
    cp -r "$TMPPATH/Filmon-main/usr/lib/enigma2/python/Plugins/Extensions/Filmon"/* "$PLUGINPATH/" 2>/dev/null
    echo "Copied from standard plugin directory"
elif [ -d "$TMPPATH/Filmon-main/usr/lib64/enigma2/python/Plugins/Extensions/Filmon" ]; then
    cp -r "$TMPPATH/Filmon-main/usr/lib64/enigma2/python/Plugins/Extensions/Filmon"/* "$PLUGINPATH/" 2>/dev/null
    echo "Copied from lib64 plugin directory"
elif [ -d "$TMPPATH/Filmon-main/usr" ]; then
    cp -r "$TMPPATH/Filmon-main/usr"/* /usr/ 2>/dev/null
...
#########################################################
#          Filmon $version INSTALLED SUCCESSFULLY        #
